...
function App() {
  ...
  useEffect(() => {
    function handlePopState() {
      ...
      if (
        ...
        nextPathname.startsWith('/maintainer/')
      )
        return;
  ...
  const repoRoute = useMemo(() => {
    ...
    return match
      ? { owner: decodeURIComponent(match[1]), name: decodeURIComponent(match[2]) }
      : null;
  }, [pathname]);
  ...
  function handleConnectWallet() {
    ...
    const address = window.prompt(
      'Enter Stellar public key to connect wallet:',
      'GB5IWBA6RTXMZSCMHFSVNL6IIZMHH5WJOH7JXZ2UTZD3VP2WBVWJJOOK'
    );
    if (!address) return;
    ...
    <div className="page-shell">
      ...
      <header className="hero">
        <div className="hero-copy">
          ...
          <p>
            Stellar Bounty Board turns backlog items into funded contribution lanes. Maintainers
            lock a reward, contributors reserve the work, and payout flows through a simple review
            lifecycle.
          </p>
      ...
      {profileContributor && (
        <RecommendedBounties recommendations={recommendations} loading={loading} />
      )}
              ...
              <label className="filter-field">
                ...
                <div className="input-with-icon">
                  ...
                  <select
                    value={statusFilter}
                    onChange={(event) =>
                      setStatusFilter(event.target.value as 'all' | BountyStatus)
                    }
                  >
            ...
            <div className="profile-earnings">
              ...
              <div className="earnings-row">
                {Array.from(contributorMetrics.releasedTotalsByAsset.entries()).length === 0 ? (
                  ...
                  Array.from(contributorMetrics.releasedTotalsByAsset.entries()).map(
                    ([asset, total]) => (
